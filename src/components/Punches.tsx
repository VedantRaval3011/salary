  ...
  const processData = (rawData: any[][]) => {
    ...
    for (let s = 0; s < sections.length; s++) {
      ...
      for (let r = sec.startRow; r < endRow; r++) {
        ...
        // Skip "Generated By" rows (from Secureye or ONtime reports)
        if (empCode.toLowerCase().includes("generated by") || empCode.toLowerCase().includes("ontime")) {
          continue;
        }
    ...
    if (allOk) {
      // Don't auto-verify, let user click the button to continue
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          {data.length > 0 && (
            <div className="flex gap-2 mt-4 pt-4 border-t border-gray-200">
              ...
              <button
                onClick={() => {
                  const allOk = data.every((r) => r.misPunch === false);
                  if (allOk) {
                    setVerified(true);
                  } else {
                ...
                className={`px-4 py-2 ${data.every((r) => !r.misPunch) ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} text-white text-sm rounded-lg font-medium transition-colors shadow-sm`}
              >
                {data.every((r) => !r.misPunch) ? "✔ Continue to Main Page" : "⚠ Ignore Mispunches and Continue"}
              </button>
            </div>
          )}
        </div>
