...
export const exportMajorMediumDifferences = (data: UnifiedComparisonRow[]) => {
  // Filter only employees with Major or Medium differences
  const filteredData = data.filter(row => {
    const hasMajorOrMedium = 
      row.presentDaysCategory === "Major" || row.presentDaysCategory === "Medium" ||
      row.lateCategory === "Major" || row.lateCategory === "Medium" ||
      row.otCategory === "Major" || row.otCategory === "Medium";
    return hasMajorOrMedium;
  });

  if (filteredData.length === 0) {
    alert("No employees with Major or Medium differences found.");
    return;
  }
  ...
  // Set the range
  ws["!ref"] = XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: filteredData.length, c: headers.length - 1 } });

  XLSX.utils.book_append_sheet(wb, ws, "Major & Medium Issues");
  XLSX.writeFile(wb, "Major_Medium_Differences.xlsx");
};
